...
def printDisplay(gameState, msg1, msg2, arr, players):
    ...
    if not arr:
        ...
        players[gameState.turn-1].lcd.lcd_custom_str(msg1, arr, LCD_LINE_1)
        players[gameState.turn-1].lcd.lcd_string(msg2, LCD_LINE_2)
...
def dealRound(gameState, teams, players):
	...
	msg2 = "player{}   Y-Cont".format(gameState.deal)
...
def checkRenig(gameState, teams, players):
	...
	if gameState.leadTurn is False:
 		printAll("P* L*", "", [gameState.roundHand[-1].suit, gameState.lead], players)
 		raw_input("ENTER")
		if gameState.roundHand[-1].suit is not gameState.lead:
			for i in range(len(players[gameState.turn-1].hand)):
				if players[gameState.turn-1].hand[i].uid is not gameState.roundHand[-1].uid:
  					print("Compared card was not just played")					
 					if players[gameState.turn-1].hand[i].suit is gameState.lead:

  						print(str(gameState.turn-1))
 						printAll("H** L*", "", [players[gameState.turn-1].hand[i].suit, players[gameState.turn-1].hand[i].val, gameState.lead], players)
 						raw_input("ENTER")
 						msg1 = "Team: {} renig".format(teams[(gameState.turn-1)%2].name)
						msg2 = ""
...
def scoreRound(gameState, teams, players):
	...
	for i in range(len(gameState.roundHand)):
		...
		if Val > tempVal:
			tempVal = Val
			winner = i
 	print(str(winner) + " winner: " + str(Val))
	return winner
...
def convertInt(gameState, i):
	...
	if gameState.roundHand[i].val is '9':
		intVal = 1
	elif gameState.roundHand[i].val is 'T':
		intVal = 2
	elif gameState.roundHand[i].val is 'J':
		intVal = 3
	elif gameState.roundHand[i].val is 'Q':
		intVal = 4
	elif gameState.roundHand[i].val is 'K':
		intVal = 5
	elif gameState.roundHand[i].val is 'A':
		intVal = 6
...
def playRound(gameState, teams, players):
	...
	for k in range(5):
		for j in range(4):
			msg2 = "Wait..."
			msg1 = "T* L* A{}/{} B{}/{}".format(teams[0].tricks, teams[0].score, teams[1].tricks, teams[1].score)
			printAll(msg1, msg2, [gameState.trump, gameState.lead], players)
			time.sleep(1)
			msg2 = "Player {} Turn".format(gameState.turn)
			msg1 = "T* L* A{}/{} B{}/{}".format(teams[0].tricks, teams[0].score, teams[1].tricks, teams[1].score)
			printDisplay(gameState, msg1, msg2, [gameState.trump, gameState.lead], players)
			time.sleep(1)
			...
			#id = input("ID: ")
 			...
 			for i in range(len(players[gameState.turn-1].hand)):
  				print(str(i))
 				if players[gameState.turn-1].hand[i].val is X.val:
 					if players[gameState.turn-1].hand[i].suit is X.suit:
  						print("card popped")
 						del players[gameState.turn-1].hand[i]
  						break
	...
	for k in range(5):
		for j in range(4):
			if players[gameState.turn-1].skip is False:
				msg2 = "Wait..."
				msg1 = "T* L* A{}/{} B{}/{}".format(teams[0].tricks, teams[1].score, teams[1].tricks, teams[1].score)
				printAll(msg1, msg2, [gameState.trump, gameState.lead], players)
				time.sleep(1)
				msg2 = "Player {} Turn".format(gameState.turn)
				msg1 = "T* L* A{}/{} B{}/{}".format(teams[0].tricks, teams[1].score, teams[1].tricks, teams[1].score)
				...
				gameState.updateRoundPlay(True)
 				...
 				for i in range(len(players[gameState.turn-1].hand)): 
 					if players[gameState.turn-1].hand[i].uid is X.uid:
 						del players[gameState.turn-1].hand[i]
				gameState.updateRoundHand(X)
